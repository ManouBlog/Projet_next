...
async function getBooks() {
  try {
    const response = await fetch('https://projet-next-sandy.vercel.app/api/books');
...
const Books =()=>{
    ...
    const handleSubmit = async(e)=>{
      e.preventDefault();
      setIsLoading(true)
      const res = await fetch('https://projet-next-sandy.vercel.app/api/books/search?query='+myQuery)
    ...
    const handleDelete = async(id)=>{
     try{
      const res = await fetch("https://projet-next-sandy.vercel.app/api/books/"+id,{
      method:"DELETE",
     })
